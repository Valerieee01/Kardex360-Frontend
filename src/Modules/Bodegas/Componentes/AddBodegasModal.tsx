...
type Props = {
  open: boolean;
  onClose: () => void;
  onSubmit?: (data: WarehouseCreateInput) => Promise<void> | void;
  loading?: boolean;
  initialData?: WarehouseCreateInput | null;
};
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
          <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold text-gray-900">
                {isEdit ? "Editar bodega" : "Crear nueva bodega"}
              </h3>
              <p className="text-sm text-gray-500">
                Registra una ubicación física para tu inventario.
              </p>
            </div>
          ...
          <div className="px-6 py-5 border-t border-gray-100 flex items-center justify-end gap-3">
            ...
            >
              {isBusy ? "Guardando..." : isEdit ? "Actualizar Bodega" : "Guardar Bodega"}
            </button>
          </div>
        </div>
      </div>
