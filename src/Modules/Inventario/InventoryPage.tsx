      ...
      <td className="px-6 py-4 text-right relative">
        ...
        {showMenu && (
          <div className="absolute right-6 mt-2 w-44 bg-white border border-gray-200 rounded-xl shadow-lg z-10">
            ...
            <button
              onClick={() => { setShowMenu(false); onClone(item); }}
              className="w-full flex items-center gap-2 px-4 py-3 text-sm hover:bg-gray-50"
            >
...
export default function InventoryPage() {
  ...
  const [selectedOriginalReference, setSelectedOriginalReference] = useState<string | null>(null);
  ...
  const handleSubmit = async (form: ProductFormValues) => {
    if (modalMode === "edit") { await handleUpdate(form); return; }
    if (modalMode === "clone") { await handleClone(form); return; }
  ...
  const modalTitle =
    ...
    : modalMode === "clone" ? "Clonar Producto"
    : "Agregar Nuevo Producto";

  const modalSubmitText =
    modalMode === "edit" ? "Actualizar Producto"
    : modalMode === "clone" ? "Guardar Clon"
    : "Guardar Producto";
  ...
  const rangeStart = Math.min((currentPage - 1) * itemsPerPage + 1, filteredProducts.length);
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <p className="text-sm text-gray-500">
                {filteredProducts.length > 0
                  ? `Mostrando ${rangeStart}–${rangeEnd} de ${filteredProducts.length} producto(s)`
                  : "0 productos"}
              </p>
